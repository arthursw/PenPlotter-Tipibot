 DropdownList connectDropList;
    ...
    Slider servoUpSlider;
    Slider servoDownSlider;
    Slider servoUpTempoSlider;
    Slider servoDownTempoSlider;
    ...
    public void createcp5GUI()
    {
      ...
      //  cp5.addFrameRate().setInterval(10).setPosition(0,height - 10).setColorValue(color(0));
       ...
       // addButton("export", "Export",leftMargin, posY+=ySpace);
        ...
        servoUpSlider = addSlider(leftMargin,posY += ySpace/2,"servoUp", "Servo up", 0.f, 3000.f, servoUpValue);
        servoDownSlider = addSlider(leftMargin,posY += ySpace/2,"servoDown", "Servo down", 0.f, 3000.f, servoDownValue);
    ...
    public void setPenPosition(ControlEvent theEvent)
    {
        Button b = (Button) theEvent.getController();
        if (b.getCaptionLabel().getText().contains("Set pen position"))
        {
            b.setCaptionLabel("Cancel");
            settingPenPosition = true;
            setPenPositionButtonJustPressed = true;
        }
        else if (b.getCaptionLabel().getText().contains("Cancel"))
        {
            b.setCaptionLabel("Set pen position");
            settingPenPosition = false;
        }
    }
    
    public void tabletMode(ControlEvent theEvent)
    {
        Button b = (Button) theEvent.getController();
        if(b.getCaptionLabel().getText().contains("Tablet mode"))
        {
    ...
    public void goHome()
    {
        com.sendAbsolute();
        com.sendPenUp();
        com.sendMoveG0(homeX, homeY+homeOffsetY);
    }
    ...
    public void servoUp(float servoUp)
    {
        servoUpValue = int(servoUp);
        if(penUpButton != null && penUpButton.getCaptionLabel().getText().contains("Pen Down") && com != null) // Servo is up
        {
            int servoUpTempoTemp = servoUpTempo;
            servoUpTempo = 0;
            com.sendPenUp();
            servoUpTempo = servoUpTempoTemp; 
        }
    }

    public void servoDown(float servoDown)
    {
        servoDownValue = int(servoDown);
        if(penUpButton != null && penUpButton.getCaptionLabel().getText().contains("Pen Up") && com != null) // Servo is down
        {
            int servoDownTempoTemp = servoDownTempo;
            servoDownTempo = 0;
            com.sendPenDown();
            servoDownTempo = servoDownTempoTemp;  
        }
    }

    public void servoUpTempo(float servoUp)
    {
        servoUpTempo = int(servoUp);
    }

    public void servoDownTempo(float servoDown)
    {
        servoDownTempo = int(servoDown);
    }
